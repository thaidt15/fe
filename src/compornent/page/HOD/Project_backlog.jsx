            ...
            <Layout>
                ...
                <Breadcrumb
                    items={[
                        ...
                        {
                            title: <Link to='../class-user'>Class User</Link>,
                        },
                        {
                ...
                <Content style={{ textAlign: 'left', padding: '20px', paddingLeft: '40px', paddingRight: '40px' }}>
                    ...
                    <Modal
                        ...
                        onOk={() => handleGradingLOCOk(currentRecord)}  // Ensure you pass the record
                        onCancel={() => setIsGradingLOCVisible(false)}
                       
                    >
                        <div>
                            <Switch checked={gradingCheckList.useMVC} onChange={checked => setGradingCheckList(prev => ({ ...prev, useMVC: checked }))} />
                            <label>Sử dụng mô hình MVC cho chương trình</label>
                        </div>
                        <div>
                            <Switch checked={gradingCheckList.useJSTL} onChange={checked => setGradingCheckList(prev => ({ ...prev, useJSTL: checked }))} />
                            <label>Sử dụng JSTL</label>
                        </div>
                        <div>
                            <Switch checked={gradingCheckList.correctPackageNaming} onChange={checked => setGradingCheckList(prev => ({ ...prev, correctPackageNaming: checked }))} />
                            <label>Các class đã đưa vào package và đặt tên package đúng - là danh từ, chữ thường, có ý nghĩa tương ứng với các lớp trong package?		</label>
                        </div>
                        <div>
                            <Switch checked={gradingCheckList.correctClassNaming} onChange={checked => setGradingCheckList(prev => ({ ...prev, correctClassNaming: checked }))} />
                            <label>Đặt tên class cho đúng - Là danh từ, Chữ cái đầu của 1 từ là chữ hoa, Tên class phải có ý nghĩa, 		</label>
                        </div>
                        <div>
                            <Switch checked={gradingCheckList.useTextFormat} onChange={checked => setGradingCheckList(prev => ({ ...prev, useTextFormat: checked }))} />
                            <label>Dùng TextFormat/NumberFormat để format  lượt view (có định dạng 6 chữ số)		</label>
                        </div>
                    </Modal>
